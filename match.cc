...
{
    this->probaEvent = rand()%101; //generer un nombre qui servira de comparatif pour l'apparition d'un evenement lors d'un match
...
int match::play_match()
{
    ...
    for(int i=0; i<2; i++)
    {
        ...
        for(int j=0; j<18; j++)
        {
            ...
            double randDou = rand()%101;
            if(randDou<=this->probaEvent)
            {
                Effet ef = event.playEvenement();
                interpreteEffet(ef,buts,this->tab_eq);
            }else{
                std::cout<<"\nPas d'évenement..."<<std::endl;
            }
            std::cout << "Temps du jeu " << std::to_string((18*i)+j) << " : " << this->tab_eq[0].getNom() << " " << std::to_string(buts[0]) << ": " << std::to_string(buts[1]) << " " << this->tab_eq[1].getNom() << std::endl;
            std::cout << "\n(Pour continuer, entrez une touche)" << std::endl;
            std::string entrer;
            std::cin>>entrer;
        }
